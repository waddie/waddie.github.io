(ns dev.tomwaddington.www.cv
  "Functions for building the CV page."
  (:require [dev.tomwaddington.www.icons :refer
             [email-link github-link linkedin-link]]
            [dev.tomwaddington.www.render :refer [format-date-MY render-page]]
            [dev.tomwaddington.www.util :refer [fetch-file-data]]))

(defn ^:private fetch-cv-data
  "Retrieve and parse the CV data."
  []
  (fetch-file-data "cv/cv.edn"))


(defn ^:private arrangements
...
(defn ^:private experience-quicklinks
...
(defn ^:private education-quicklinks
...
(defn ^:private nav
  "Render shortcut nav."
  [data]
  [:nav {:class "cv"} github-link linkedin-link email-link
